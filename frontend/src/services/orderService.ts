...
export const getOrdersByCustomerId = async (customerId: string): Promise<OrderResponse[]> => {
  const response = await apiClient.get<OrderResponse[]>(`/api/v1/orders/customer/${customerId}`);
  return response.data;
};
