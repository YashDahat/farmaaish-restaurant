...
import type { CreateMenuItemRequest, MenuItemCategoryDto, MenuItemDto } from '@/types/menu';
...
export const getMenuItemCategoryById = async (id: string): Promise<MenuItemCategoryDto> => {
  const response = await apiClient.get<MenuItemCategoryDto>(`/api/v1/menu/categories/${id}`);
  return response.data;
};

export const createMenuItem = async (request: CreateMenuItemRequest): Promise<MenuItemDto> => {
  const response = await apiClient.post<MenuItemDto>('/api/v1/admin/menu/items', request);
  return response.data;
};

export const updateMenuItem = async (id: string, request: CreateMenuItemRequest): Promise<MenuItemDto> => {
